...
const Tasks = (props) => {
  ...
  return (
    <Fragment>
          ...
          tasks?.map((item) => (
            ...
            >
              <div className="d-flex justify-content-between w-100">
                <div className={active ? 'mb-1 text-white' : 'mb-1'} style={{ fontWeight: 800 }}>
                  {item?.taskName}
                </div>
                <div>
                  {item?.freeze?.length > 0 && (
                    <RiAlarmWarningLine
                      onClick={(e) => {
                        e.preventDefault();
      ...
      <FreezeTask isOpen={freezeTaskModal} setIsOpen={setFreezeTaskModal} data={selTask} />
      <Modal
