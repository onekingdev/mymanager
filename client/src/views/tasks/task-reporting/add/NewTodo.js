...
import { RiNumber0, RiNumber1, RiNumber5, RiNumber9 } from 'react-icons/ri';
...
import CodeSelInLib from './CodeSelectInLibrary';
...
const NewTodo = (props) => {
  ...
  const [codeSelectModal, setCodeSelectModal] = useState(false);
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [zoomViewURL, setZoomViewURL] = useState(false);
  const { saveCheckList } = useSelector((state) => state.tasks);
  const { qrcodeList } = useSelector((state) => state.qrcode);
  const barcodeList = [];
  ...
  function checkListSaveHandler() {
    ...
    socket.emit('newChecklistForYou', {
      ...selectedTask,
      employerInfo: {
  ...
  function handleSelCodeInLib(i) {
    setSelectedIndex(i);
    setCodeSelectModal(true);
  }

  function handleZoomBtn(i) {
    setSelectedIndex(i);
    setZoomViewURL(todos[i]?.codeURL);
  }

  if (!selectedTask) {
    return (
      <Fragment>
        <div className="mb-2 d-flex justify-content-between align-items-center empty-list">
          <div
            style={{
              width: '100%',
              // background: '#ddd',
  ...
  return (
    <Fragment>
      ...
      <div>
        <Row className="justify-content-between align-items-center">
          <Col md={4} className="mb-md-0 mb-1">
            <Label className="form-label" style={{ fontSize: '14px', fontWeight: 'bold' }}>
              Title
            </Label>
          </Col>
              ...
              <Tag key={i}>
                <Form>
                  <Row className="justify-content-between align-items-center">
                    ...
                    {/* Code Content */}
                    <Col md={2} className="mb-md-0 mb-1">
                      {todos[i]?.proofType == 'qrCode' || todos[i]?.proofType == 'barCode' ? (
                        <Button.Ripple
                          className="btn-icon"
                          color="flat-dark"
                          onClick={(e) => {
                            e.preventDefault();
                            handleSelCodeInLib(i);
                          }}
                        >
      ...
      <CodeSelInLib
        open={codeSelectModal}
        setOpen={setCodeSelectModal}
        codeData={qrcodeList}
        selectedIndex={selectedIndex}
        todos={todos}
        setTodos={setTodos}
      />
      <ImageZoom setIsOpen={setZoomViewURL} srcUrl={zoomViewURL} />
      <Modal
              ...
              <ButtonGroup className="mb-1">
                ...
                >
                  <div className="cursor-pointer">
                    <RiNumber1 size={18} className="mb-30" />
                    <RiNumber0 size={18} className="mb-30" style={{ marginLeft: '-8px' }} />
                  </div>
