...
const PastDueClient = (props) => {
  ...
  const { setSelectedWorkingCheckList, activeTab, selectDate } = props;
  ...
  useMemo(() => {
    if (activeTab === 'past-due') {
      dispatch(workingTaskListPastDueAction({ ...initialOptions, selectDate: selectDate }));
    }
      ...
      {pastDueTasks.length === 0 ? (
        <ListGroup>
          <ListGroupItem
            ...
            tag="a"
            action
            className={active ? 'bg-primary empty-list' : 'bg-white empty-list'}
            style={{
              textAlign: 'center',
              border: 'none',
              display: 'flex',
