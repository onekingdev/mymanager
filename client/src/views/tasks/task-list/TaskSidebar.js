...
const TaskSidebar = (props) => {
  ...
  const selectedWorkspaceId = useSelector((state) => state?.workspace?.selectedWorkspace?._id);
  ...
  const labelOptions = store
    ...
    : null;
  const assigneeOptions = contactList
    ? contactList?.map((x) => {
        return { value: x._id, label: x.fullName, img: x.photo };
      })
    : null;
    ...
    if (!isObjEmpty(selectedTask)) {
      ...
      if (selectedTask.assignedTo.length) {
        ...
        selectedTask.assignedTo.map((assignee) => {
          arr.push({ value: assignee.contactId, label: assignee.title, img: assignee.img });
        });
        setAssignedTo(arr);
      }
  ...
  const onSubmit = (data) => {
    if (data.title.length) {
      ...
      if (assignedTo !== null) {
        assignedTo.map((item) => {
          assignedArr.push({ contactId: item.value, title: item.label, img: item.img });
        });
      }
      ...
      dispatch(updateTask({ postData, workspaceId: selectedWorkspaceId })).then(() => {
