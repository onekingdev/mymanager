...
const PreviewTemplate = ({ eventForm, stepper, activeView, eventInfo }) => {
  // ** Store variable
  ...
  const [selectedTemplate, setSelectedTemplate] = useState(
    eventInfo.templateIndex ? eventInfo.templateIndex : 0
  );
  ...
  const handleCreate = () => {
    ...
    const response = dispatch(updateEvent({ _id: eventInfo._id, eventForm }));
    if (response) success('Event is successfully updated');
    history.push('/events');
  };
