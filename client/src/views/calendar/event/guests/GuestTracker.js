...
const GuestTracker = (props) => {
  ...
  const completedSeats = useMemo(() => (came / guests.length).toFixed(2) * 100, [coming, guests]);
  ...
  const options = {
    ...
    series = [completedSeats ? completedSeats.toFixed(0) : 0];
      ...
      <CardBody>
        ...
        <div className="d-flex justify-content-between mt-1">
          <div className="text-center">
            ...
            <span className="font-large-1 fw-bold">{coming}</span>
          </div>
          <div className="text-center">
            <CardText className="mb-50">Next Time</CardText>
            <span className="font-large-1 fw-bold">{maybe}</span>
          </div>
          <div className="text-center">
            <CardText className="mb-50">No Reply</CardText>
            <span className="font-large-1 fw-bold">{noReply}</span>
          </div>
        </div>
      </CardBody>
    </Card>
