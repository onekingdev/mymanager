...
const inviteOptions = [
  { value: '640264d8c582c2fc9275061a', label: 'Ryan' },
  { value: '640264d8c582c2fc9275061b', label: 'Ajay' },
  { value: '640264d8c582c2fc9275061c', label: 'Clinton Oh' },
  { value: '640264d8c582c2fc9275061d', label: 'Diwakar' },
  { value: '640264d8c582c2fc9275061e', label: 'Akshit' }
];
...
const AddAppointment = (props) => {
  ...
  const [invitedList, setInvitedList] = useState(
    inviteOptions
      ? invitedUserId
        ? inviteOptions.find((x) => x.value == invitedUserId.toString())
        : inviteOptions[0]
      : {}
  );
  ...
  const initState = () => {
    ...
    setRemindTo('');
    setNotification(notificationOptions[3]);
  };
  ...
  const createAppointmentBtnClicked = (e) => {
    e.preventDefault();
    ...
    const data = {
      ...
      extendedProps: {
        calendar: calendarEventType == 'Appointment' ? 'Appointments' : 'Classes'
      }
    };
    dispatch(createAppointment(data))
      .then((res) => {
        ...
        if (res?.payload?.status == 200) {
          ...
          initState();
          setAddAppointmentSidebarOpen(false);
        } else {
          error('Update failed');
        }
      })
  ...
  return (
    ...
    >
      <ModalHeader
        ...
        tag="div"
      >
        <h5 className="modal-title">Create {calendarEventType}</h5>
      </ModalHeader>
      <PerfectScrollbar options={{ wheelPropagation: false }}>
        <ModalBody className="flex-grow-1 pb-sm-0 pb-3">
          <Form>
            <Row>
              ...
              <Col sm={12} md={12} lg={12}>
                <div className="mb-1">
                  <Label className="form-label" for="remindTo">
        ...
        <ModalFooter>
          <Button
            ...
            type="submit"
            color="primary"
            onClick={createAppointmentBtnClicked}
          >
            Create
          </Button>
          <Button
