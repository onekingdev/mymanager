...
const CategorySidebar = ({
  ...
  useEffect(() => {
    const newCategoryList = {
        'Sales': categoryData.filter(item=>item.type === 'sales'),
        'Email': categoryData.filter(item=>item.type === 'email'),
        'Leads': categoryData.filter(item=>item.type === 'leads'),
        'Forms': categoryData.filter(item=>item.type === 'forms'),
        'Website': categoryData.filter(item=>item.type === 'website'),
    }
    setNewCategoryData(newCategoryList);
  }, [categoryData])
        ...
        <div className="task-app-menu h-100 d-flex flex-column">
          ...
          >
            ...
            <div className="jrnl_wrapper_sidebar">
                ...
                {Object.entries(newCategoryData).map(([key, categoryList]) => (
                    <>
                        {!!categoryList.length &&
                            <>
                                <h5 className='m-0 ps-1' key={key}><strong>{key}</strong></h5>
          ...
          <div
            className={`${noAction?'hidden':''} project-create-workspace-btn my-1 btn text-center`}
            style={{ width: '100%' }}
          >
