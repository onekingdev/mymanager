...
const NoteModal = ({ row, rowId, toggle, isOpen, notes, dispatch, setDeleteModal }) => {
  ...
  const handleNoteSave = (e) => {
    ...
    if (notedata.followUpType != '' && notedata.response != '' && notedata.note != '') {
      dispatch(MemberNoteAddAction(notedata, rowId, row?._id));
  ...
  const handleDeleteModal = async (data) => {
    ...
    if (res.value) {
      dispatch(MemberNoteDeleteAction(data?._id));
