...
import {
  VendorNoteAddAction,
  VendorNoteEditAction,
  VendorNoteDeleteAction
} from '../store/actions';
...
const NoteModal = ({ row, toggle, isOpen, notes, dispatch, setDeleteModal }) => {
  ...
  const handleNoteSave = (e) => {
    ...
    if (notedata.followUpType != '' && notedata.response != '' && notedata.note != '') {
      dispatch(VendorNoteAddAction(notedata));
  ...
  const handleNoteUpdate = (e) => {
    e.preventDefault();
    dispatch(VendorNoteEditAction(editModalData));
    toggle2();
  ...
  const handleDeleteModal = async (data) => {
    ...
    if (res.value) {
      dispatch(VendorNoteDeleteAction(data?._id));
