...
import { Plus, Check, X, Pause } from 'react-feather';
...
const PaymentMethods = ({ selectedUser }) => {
  ...
  const addCardHandler = async () => {
    ...
    if (cardNumber === '') {
      ...
      if (_id === '') {
        // Add New Payment Method to current contact
        delete card['_id'];

        let tmpPaymentMethod = [...selectedUser?.paymentMethods];
        if (tmpPaymentMethod.length) {
          tmpPaymentMethod.push(card);
        } else {
          tmpPaymentMethod = [card];
        }

        const tmpContact = {
          ...selectedUser,
          paymentMethods: tmpPaymentMethod
        };

        await mutate(tmpContact);

        initCard();
      } else {
        // Update Payment Method to current contact
        let tmpPaymentMethod = [...selectedUser?.paymentMethods];

        const updatedPayemntMethod = tmpPaymentMethod?.map((each) =>
          each?._id == _id ? card : each
        );

        const tmpContact = {
          ...selectedUser,
          paymentMethods: updatedPayemntMethod
        };
        await mutate(tmpContact);
  ...
  const deleteCardHandler = async (payload) => {
    const result = await confirm('Are you sure?', {
      closeOnOverlayClick: true,
      classNames: 'custom_confirm_box'
    });
    if (result) {
      // Delete Payment Method from current contact
      const { id } = payload;
      let tmpPaymentMethod = [...selectedUser?.paymentMethods];

      const updatedPayemntMethod = tmpPaymentMethod?.filter((each) => each?._id !== id);
                    ...
                    <div className="d-flex justify-content-between flex-sm-row flex-column">
                      <div className="card-information">
                        <img
                          ...
                          src={cardsObj[String(detectCardType(parseInt(card.cardNumber)))]}
                          alt={card?.imgAlt}
