...
import {
  RelationNoteAddAction,
  RelationNoteEditAction,
  RelationNoteDeleteAction
} from '../store/actions';
import withReactContent from 'sweetalert2-react-content';
import Swal from 'sweetalert2';
import AddAppointment from '../../../calendar/appointment/AddAppSidebar';

const NoteModal = ({ row, toggle, isOpen, notes, dispatch, setDeleteModal }) => {
  ...
  const handleNoteSave = (e) => {
    ...
    if (notedata.followUpType != '' && notedata.response != '' && notedata.note != '') {
      dispatch(RelationNoteAddAction(notedata));
  ...
  const handleNoteUpdate = (e) => {
    e.preventDefault();
    dispatch(RelationNoteEditAction(editModalData));
    toggle2();
  ...
  const handleDeleteModal = async (data) => {
    ...
    if (res.value) {
      dispatch(RelationNoteDeleteAction(data?._id));
      ...
      >
        ...
        <ModalBody>
          <Row>
            ...
            <Col xl="7" lg="7">
              <DataTable columns={columnsdata} data={notes?.data} pagination />
            </Col>
          </Row>
        </ModalBody>
      </Modal>
