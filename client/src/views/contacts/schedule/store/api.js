...
export const getAllEmployeeShift = () => {
  return API.get('/employee-shifts/');
};

export const shiftAdd = (newShift) => {
  API.post(`/employee-shifts/`, newShift).then((response) => {
    if (response.status == 200 || response.status == 201) {
      toast.success('Create shifts successfully');
    } else {
...
export const shiftUpdate = ({ id, start, end, weekDay, note, isOpen }) => {
  API.post(`/employee-shifts/${id}`, { start, end, weekDay, note, isOpen }).then((response) => {
    if (response.status == 200 || response.status == 201) {
    } else {
      toast.error('Failed to save shift');
    }
...
export const saveBudget = (payload) => {
  API.post('/employee-budgets', payload).then((response) => {
    if (response.status == 200 || response.status == 201) {
...
export const getSalesProjected = () => {
  return API.get('/employee-budgets');
};
