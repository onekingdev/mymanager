...
export default function BtnMembershipStartDate({ customField }) {
    // ** Contexts
  const {
    recipients,
    board,
    ...
    if (shouldDrop) {
      if (customField) {
        setItem({
          ...
          id: board.length + 1,
          dataLabel: `Start Date ${board.length + 1}`,
          page: currentPage,
        ...
        setItem({
          type: 'membershipStartDate',
          color: recipients.filter((x) => x.active === true)[0]?.color,
          _type: 'btn',
          title: 'Membership StartDate',
          icon: <Users />,
          formatting: 100,
          tooltip: '',
          required: false,
          ...
          top: (e.pageY - src.y) / zoom,
          id: board.length + 1,
          dataLabel: `Membership StartDate ${board.length + 1}`,
          page: currentPage,
          recipient: recipients.find((x) => x.active === true),
