...
export default function BtnMembershipFreequency({ customField }) {
    // ** Contexts
  const {
    recipients,
    board,
    ...
    if (shouldDrop) {
      if (customField) {
        setItem({
          ...
          id: board.length + 1,
          dataLabel: `Membership Freequency ${board.length + 1}`,
          page: currentPage,
        ...
        setItem({
          type: 'membershipFreequency',
          color: recipients.filter((x) => x.active === true)[0]?.color,
          _type: 'btn',
          title: 'Membership Freequency',
          icon: <Users />,
          formatting: 100,
          tooltip: '',
          required: false,
