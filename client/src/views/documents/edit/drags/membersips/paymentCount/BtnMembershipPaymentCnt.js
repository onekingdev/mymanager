...
import { Type, Users } from 'react-feather';
import { Button } from 'reactstrap';
import { DocumentContext } from '../../../../../../utility/context/Document';

export default function BtnMembershipPaymentCount({ customField }) {
    // ** Contexts
  const {
    recipients,
    board,
    ...
    if (shouldDrop) {
      if (customField) {
        setItem({
          ...
          id: board.length + 1,
          dataLabel: `No of Payments ${board.length + 1}`,
          page: currentPage,
        ...
        setItem({
          type: 'membershipPaymentCount',
          color: recipients.filter((x) => x.active === true)[0]?.color,
          _type: 'btn',
          title: 'Membership No. of Payments',
          icon: <Users />,
          formatting: 100,
          tooltip: '',
          required: false,
          ...
          top: (e.pageY - src.y) / zoom,
          id: board.length + 1,
          dataLabel: `Membership No. of payments ${board.length + 1}`,
          page: currentPage,
          recipient: recipients.find((x) => x.active === true),
