...
export default function BtnMembershipNextPayment({ customField }) {
    // ** Contexts
  const {
    recipients,
    board,
    ...
    if (shouldDrop) {
      if (customField) {
        setItem({
          ...
          id: board.length + 1,
          dataLabel: `End Date ${board.length + 1}`,
          page: currentPage,
        ...
        setItem({
          type: 'membershipNextPayment',
          color: recipients.filter((x) => x.active === true)[0]?.color,
          _type: 'btn',
          title: 'Membership NextPayment',
          icon: <Users />,
          formatting: 100,
          tooltip: '',
          required: false,
          ...
          top: (e.pageY - src.y) / zoom,
          id: board.length + 1,
          dataLabel: `Membership NextPayment ${board.length + 1}`,
          page: currentPage,
          recipient: recipients.find((x) => x.active === true),
