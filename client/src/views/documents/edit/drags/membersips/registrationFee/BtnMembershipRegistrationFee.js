...
export default function BtnMembershipRegistrationFee({ customField }) {
    // ** Contexts
  const {
    recipients,
    board,
    ...
    if (shouldDrop) {
      if (customField) {
        setItem({
          ...
          id: board.length + 1,
          dataLabel: `Registration fee ${board.length + 1}`,
          page: currentPage,
        ...
        setItem({
          type: 'membershipregistrationFee',
          ...
          top: (e.pageY - src.y) / zoom,
          id: board.length + 1,
          dataLabel: `Membership registration ${board.length + 1}`,
          page: currentPage,
          recipient: recipients.find((x) => x.active === true),
