...
export default function BtnMembershipDurationType({ customField }) {
    // ** Contexts
  const {
    recipients,
    board,
    ...
    if (shouldDrop) {
      if (customField) {
        setItem({
          ...
          id: board.length + 1,
          dataLabel: `Membership DurationType ${board.length + 1}`,
          page: currentPage,
        ...
        setItem({
          ...
          _type: 'btn',
          title: 'Membership Duration Type',
          icon: <Users />,
          formatting: 100,
          tooltip: '',
          required: false,
          ...
          top: (e.pageY - src.y) / zoom,
          id: board.length + 1,
          dataLabel: `Membership Duration Type ${board.length + 1}`,
          page: currentPage,
          recipient: recipients.find((x) => x.active === true),
