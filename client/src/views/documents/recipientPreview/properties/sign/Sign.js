...
export default function Sign({ item, isDone, recipient }) {
  //** States */
  const [isOpen, setIsOpen] = useState(false);

  const { hashcode,isTemplate } = useContext(DocumentContext);

  const toggle = () => {
    if(recipient===undefined){
      if (!(item.recipient.hashCode != hashcode && item.isDone === true)) {
        if (isDone === false || isDone===undefined) {
          if (!item.readOnly) {
      ...
      if (!(item.recipient.hashCode != recipient.hashCode && item.isDone === true)) {
        if (isDone === false || isDone===undefined) {
          if (!item.readOnly) {
  ...
  return (
    <>
      ...
      {isTemplate!==true && <SignModal toggle={toggle} open={isOpen} type="signature" item={item} recipient={recipient}/>}
    </>
  );
}
