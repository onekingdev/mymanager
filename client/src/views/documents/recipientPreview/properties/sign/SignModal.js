...
export default function SignModal({ toggle, open, type, item,recipient }) {
  // ** States
  ...
  const handleShowSignature = () => {
    ...
    board.map((b) => {
      ...
      if(recipient===undefined){
        ...
        if (x.recipient.hashCode===recipient.hashCode && x.type === 'sign') {
       
          x.isDone = true;
        }
        if(sign.initials?.path){
          if (x.recipient.hashCode===recipient.hashCode && x.type === 'initial') {
