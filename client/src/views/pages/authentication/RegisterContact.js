...
const RegisterContact = () => {
  ...
  const { sendContactSignupOtp, getVisitorInfo, contactRegistrationReqeust } = useAuth();
  ...
  const onSubmit = () => {
    ...
    if (isOtpSended) {
      ...
      setIsLoading(true);
      contactRegistrationReqeust({ ...data, contactTypeId, assignerId, path })
        .then((response) => {
          setIsLoading(false);
        ...
        .catch((err) => {
          setIsLoading(false);
          toast.error(<ToastContent message={String(err.message)} />);
        });
    ...
    if (Object.values(tempData).every((field) => field.length > 0) && data.terms === true) {
      setIsLoading(true);
      sendContactSignupOtp({ ...data, countryCode, contactSignup: true })
        .then((response) => {
          setIsLoading(false);
          setIsOtpSended(true);
        })
