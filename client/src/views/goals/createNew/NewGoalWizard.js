...
import AddHabit from '../habit-view/AddHabit';

const NewGoalWizard = ({workspaceId,toggle}) => {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: (
        <Fragment>
          {rSelected === 1 ? (
            <AddGoal toggle={toggle} workspaceId={workspaceId} stepper={stepper} type="wizard-modern" />
          ) : rSelected === 2 ? (
            <AddHabit toggle={toggle} workspaceId={workspaceId} stepper={stepper} type="wizard-modern" />
          ) : (
