...
export const updateTickets = createAsyncThunk(
  ...
  async ({ ticketIds, dataToUpdate }, { dispatch, getState }) => {
    ...
    if (response.status) {
      toast.info('Successfully change ticket status', { autoClose: 2000, icon: false });
    } else {
      console.log('error');
    }
...
export const deleteTickets = createAsyncThunk(
  'appTicket/delete',
  async ({ ticketIds }, { dispatch, getState }) => {
  ...
  reducers: {
    ...
    selectTicket: (state, action) => {
      ...
      if (!selectedTickets.includes(action.payload)) {
        ...
        selectedTickets.splice(selectedTickets.indexOf(action.payload), 1);
      }
    ...
    selectAllTickets: (state, action) => {
      ...
      if (action.payload.checked) {
        ...
        state.tickets
          .filter((ticket) => ticket.status.toLowerCase() === action.payload.status)
          .forEach((ticket) => selectAllTicketsArr.push(ticket._id));
      } else {
        selectAllTicketsArr.length = 0;
      }
    ...
    selectCurrentTicket: (state, action) => {
      state.currentTicket = state.tickets.filter((ticket) => ticket._id === action.payload)[0];
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getTickets.fulfilled, (state, action) => {
        let curTicket = null;
        if (state.currentTicket !== null && state.currentTicket !== undefined) {
