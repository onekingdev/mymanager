...
const SidebarLeft = (props) => {
  ...
  const [rightSelectedEv, setRightSelectedEv] = useState();
  ...
  const filterByOptions = [
    { value: 'all', label: 'All', number: 0 },
    { value: 'employee', label: 'Employee', number: 1 },
    { value: 'relationship', label: 'Relationship', number: 2 },
    { value: 'client', label: 'Client', number: 3 },
    { value: 'vendor', label: 'Vendor', number: 4 },
    { value: 'lead', label: 'Lead', number: 5 }
  ];
  ...
  const archiveHandler = (selectedRow) => {
    handleUserClick(rightSelectedEv, selectedRow);
  ...
  const handleUserClick = (e, item) => {
    setIsLoading(true);
    document.querySelectorAll('.chat-sidebar-link').forEach((el, index) => {
    ...
    const response = dispatch(selectChat(item._id));
    response.then((data) => {
      setIsLoading(false);
    });
    setActive(item._id);
    ...
    <div className="sidebar-left chat-sidebar-left">
      <div className="sidebar chat-sidebar">
        ...
        >
          ...
          >
            ...
            <div className="d-flex justify-content-between align-items-center p-2">
              ...
              <Select
                ...
                isClearable={false}
                className="react-select w-50 filter-by-contacts"
          ...
          {/* Richt Click Menu */}
          <Menu id="menu-id">
            <Item disabled={selectedRow?.isClosed == true ? true : false}>
              <div className="w-100 h-100" onClick={() => archiveHandler(selectedRow)}>
