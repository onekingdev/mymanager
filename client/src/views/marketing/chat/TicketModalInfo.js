...
const TicketModalInfo = ({ store, dispatch, setIsCreateTicketModalOpen, selectedTicket }) => {
  ...
  const [status, setStatus] = useState({ value: 'medium', label: 'Select...' });
  ...
  useEffect(() => {
    let html = [];
    setMessageData(formattedChatData());
    formattedChatData().forEach((item, index) => {
  ...
  const handleCreateTicket = (e) => {
    ...
    let data,
      tmpMessages = [];
    if (!isMessageAll) {
      messageData[messageData.length - 1]?.messages.length > 0
        ? tmpMessages.push({
