...
function MessageInput() {
  ...
  const handaleSendingsms = async (e) => {
    ...
    if (inputvalue.length == 0) {
      ...
      //socket.emit('adminRegister', user.id);
    ...
    <>
      ...
      <div style={{margin: "10px"}}>
        <Form className="d-flex align-items-center gap-1">
          ...
          <Mic className="cursor-pointer" size={20} onClick={(e) => recordMessage()} />
          <Image className="cursor-pointer text-secondary" size={20} />
          <input type="file" id="attach-doc" hidden />
          <Button
            className="send d-flex gap-1"
            color="primary rounded-1"
            onClick={(e) => handaleSendingsms(e)}
          >
            <Send size={16} />
