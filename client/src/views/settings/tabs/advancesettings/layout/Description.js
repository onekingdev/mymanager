...
import { getCustomersWithSmartList } from '../store';
...
function Description(props) {
  ...
  const relationshipData = totalStore.relationshipContacts.list
    ? totalStore.relationshipContacts.list
    : [];
  ...
  useEffect(() => {
    dispatch(getCustomersWithSmartList(descriptiondetails));
      ...
      <div className="m-2">
        <Card className="my-2">
          <CardBody className="rounded-none">
            <CardTitle tag="h5">ContactType</CardTitle>
            <CardText>
        ...
        <Card className="my-2">
          <CardBody className="rounded-none">
            ...
            <CardText>{descriptiondetails.creteria}</CardText>
          </CardBody>
        </Card>
      </div>
    </>
