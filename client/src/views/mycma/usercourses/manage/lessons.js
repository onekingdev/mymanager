...
// ** Reactstrap Imports
import { Card, CardBody, CardText, Button, Badge, Col, Row } from 'reactstrap';
...
const Lessons = (props) => {
    ...
    const products = store?.courseList
    // ** Props
    const {
        // store,
        type,
    ...
    const renderProducts = () => {
        if (products.length) {
            return products.map((item) => {
                ...
                return (
                    <Col lg="3">
                        <Card className="ecommerce-card" key={item.name} onClick={() => setDetails({ show: !details.show, details: item })}>
    ...
    return (
        ...
        // >
        <Row>
            {!details?.show ? renderProducts() : <Details setDetails={setDetails} details={details}></Details>}
        </Row>
    );
};
