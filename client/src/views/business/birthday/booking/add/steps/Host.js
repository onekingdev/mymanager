...
const Host = ({ stepper, eventForm, row }) => {
  // ** Default Form Value
  const defaultValues = {
    hostName: '',
    hostEmail: '',
  ...
  useEffect(() => {
    setValue('hostName', row.fullName ? row.fullName : '');
    setValue('hostEmail', row.email ? row.email : '');
    setValue('hostMobileNumber', row.phone ? row.phone : '');
  }, [row]);
        ...
        <Row>
          ...
          <Col md="6" className="mb-1">
            ...
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <Input autoFocus type="text" placeholder="Enter Email" value={value} disabled />
              )}
            />
            {errors.hostEmail && (
              <FormText color="danger" id="validation-add-board">
